...
// Helper: Process loan repayments from CBS data — match to schedules, update paidAmount/status
const processLoanRepayments = async (cbsData) => {
  ...
  for (const record of cbsData) {
    const accountNumber = String(record.accountNumber || record['Account Number'] || record.account_id || '').trim();
    const cbsAmount = parseFloat(record.amount || record.Amount || 0);
    ...
    // Find the oldest unpaid installment for this loan
    const unpaid = await prisma.loanSchedule.findFirst({
      where: {
        accountId: account.id,
        status: { in: ['Pending', 'Partial'] },
        expectedDate: { lte: new Date(transactionDate) },
      },
    ...
    const newPaidAmount = (unpaid.paidAmount || 0) + cbsAmount;
  ...
  return { schedulesUpdated, schedulesAutoGenerated };
};
