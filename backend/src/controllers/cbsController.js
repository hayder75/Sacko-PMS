...
// - Matches payment to oldest unpaid installment (including Missed)
// - Updates paidAmount / status on LoanSchedule
// - Rolls over excess payment to next installment(s)
// - Marks loan as completed when all installments are Paid
// Note: current_balance is already updated by updateAccountBalances from the CBS file,
// so we do NOT decrement it here (the CBS balance field is authoritative).
const processLoanRepayments = async (cbsData) => {
  ...
  for (const record of cbsData) {
    ...
    // Apply payment across installments (oldest unpaid first, including Missed)
    ...
    // Check if all installments are Paid → loan completed
    const allPaid = await prisma.loanSchedule.count({
