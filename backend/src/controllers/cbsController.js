...
// Helper: Process loan repayments from CBS data
// - Matches payment to oldest unpaid installment(s)
// - Updates paidAmount / status on LoanSchedule
// - Decreases current_balance on the loan account
// - Marks account as completed (Closed) when fully paid
// - Rolls over excess payment to next installment(s)
const processLoanRepayments = async (cbsData) => {
  let schedulesUpdated = 0;
  let schedulesAutoGenerated = 0;
  let loansCompleted = 0;

  for (const record of cbsData) {
    const accountNumber = String(record.accountNumber || record['Account Number'] || record.account_id || '').trim();
    let cbsAmount = parseFloat(record.amount || record.Amount || 0);
    const transactionDate = record.transactionDate || record['Transaction Date'] || new Date();

    if (!accountNumber || cbsAmount <= 0) continue;

    const account = await prisma.accountMapping.findUnique({
      where: { accountNumber },
      select: { id: true, accountType: true, current_balance: true },
    });

    if (!account || account.accountType !== 'Loan') continue;

    // Check if schedules exist; auto-generate if missing
    const existingCount = await prisma.loanSchedule.count({
      where: { accountId: account.id },
    });

    if (existingCount === 0) {
      const { autoGenerateLoanSchedules } = await import('../utils/performanceCalculator.js');
      const result = await autoGenerateLoanSchedules(account.id);
      schedulesAutoGenerated += result.generated || 0;
    }

    let remainingAmount = cbsAmount;

    // Apply payment across installments (oldest unpaid first), handling rollover
    while (remainingAmount > 0) {
      const unpaid = await prisma.loanSchedule.findFirst({
        where: {
      ...
      if (!unpaid) break; // no more installments to pay

      const due = unpaid.expectedAmount - (unpaid.paidAmount || 0);
      const payAmount = Math.min(remainingAmount, due);
      const newPaidAmount = (unpaid.paidAmount || 0) + payAmount;
      const newStatus = newPaidAmount >= unpaid.expectedAmount ? 'Paid' : 'Partial';
      const today = new Date();

      const dpd = Math.max(0, Math.floor((today.getTime() - new Date(unpaid.expectedDate).getTime()) / (1000 * 60 * 60 * 24)));
      ...
      await prisma.loanSchedule.update({
        ...
        data: {
          ...
          status: newStatus,
          paidDate: new Date(transactionDate),
          daysPastDue: newStatus === 'Paid' ? 0 : dpd,
        },
      });
      schedulesUpdated++;
    ...
    // Decrease current_balance by the amount actually applied
    const appliedAmount = cbsAmount - remainingAmount;
    if (appliedAmount > 0) {
      const newBalance = Math.max(0, (account.current_balance || 0) - appliedAmount);
      await prisma.accountMapping.update({
        where: { id: account.id },
        data: { current_balance: newBalance },
      });

      // Loan completed if balance is 0 (or very close)
      if (newBalance <= 0) {
        await prisma.accountMapping.update({
          where: { id: account.id },
  ...
  return { schedulesUpdated, schedulesAutoGenerated, loansCompleted };
};
...
const validateCBS = async (cbsData, branchId, validationDate, validationId) => {
  ...
  for (const cbsRecord of cbsData) {
    ...
    const cbsProductName = String(cbsRecord.product || cbsRecord.Product || cbsRecord.productName || cbsRecord['Product Name'] || '').trim();
    const mappedTaskType = CBS_PRODUCT_TO_CATEGORY[cbsProductName];

    if (!accountNumber) continue;

    // Match by accountNumber + taskType (from CBS product name) + amount within tolerance
    const matchingTask = tasks.find(t => {
      if (t.accountNumber !== accountNumber) return false;
      if (mappedTaskType && t.taskType !== mappedTaskType) return false;
      return Math.abs(t.amount - cbsAmount) < 0.01;
    });
    ...
    if (matchingTask) {
      ...
      // Propagate balance update to account mapping
      if (matchingTask.accountId) {
        const accountBalance = parseFloat(cbsRecord.balance || cbsRecord.Balance || cbsRecord.current_balance || 0);
        if (accountBalance > 0) {
          await prisma.accountMapping.update({
            where: { id: matchingTask.accountId },
            data: { current_balance: accountBalance },
          });
        }
      }
    } else {
...
export const uploadCBS = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const validationDateParsed = validationDate ? new Date(validationDate) : new Date();

    const validation = await prisma.cBSValidation.create({
      data: {
        branchId: branch.id,
        validationDate: validationDateParsed,
    ...
    // Phase 3: Process loan repayments + generate NPL snapshot
    const loanResult = await processLoanRepayments(cbsData);
    let nplSnapshot = null;
    try {
      const { generateNplSnapshot } = await import('../utils/performanceCalculator.js');
      nplSnapshot = await generateNplSnapshot(branch.id);
    } catch (e) {
      // NPL snapshot is non-critical
    }
    ...
    res.status(201).json({
      ...
      data: {
        validation: { ...updatedValidation, _id: updatedValidation.id },
        balanceUpdate,
        autoMapped: autoMapped.length,
        loanRepayments: loanResult,
        nplSnapshot,
      }
    });
  } catch (error) {
